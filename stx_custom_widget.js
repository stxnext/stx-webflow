function init() {
    ...
    var xxtraffitxxAppend = function (parent, text) {
      ...
      var elements = tempWrapperBody.querySelectorAll(
        "#xxtraffitxxTempWrapper > *"
      );
    ...
    var limit = Math.max(
      1,
      isWithinAts
        ? scriptConfig.careerPageJobPostLimit
        : scriptConfig.jobPostLimit
    );
    ...
    if (scriptConfig.fontFamily) {
      ...
      var googleFonts = ["Lato", "Roboto", "Poppins", "Open Sans"].map((font) =>
        font.toLowerCase()
      );
    ...
    if (
      scriptConfig.customTraffitRowString &&
      scriptConfig.customTraffitRowString.length > 0
    ) {
    ...
    var filtersInterior =
      ...
      `<div class="traffit-xx-select">
              ...
              <select name="query" class="traffit-xx-flex-big traffit-xx-input"/>
                <option disabled selected value> -- select an option -- </option>
              </select>
    ...
    var paginationInterior =
      "<div onclick='xxtraffixxlist.changePage({{page}})' class='{{class}} '>{{page}}</div>";
    var paginationInteriorSeparator =
      "<div class='xxxseparatorxxxtraffit'>...</div>";
    ...
    var handleFiltersResponse = function (response) {
      ...
      const TRAFFIT_JOB_CATEGORY = [
        ...new Set(TRAFFIT_DATA.items.map((item) => item.name)),
      ];
  
      const TRAFFIT_JOB_LOCATION = {};
        ...
        .forEach((item) => {
          ...
          const key2 =
            TRAFFIT_JOB_LOCATION[
              `${item.job_location.country}, ${item.job_location.locality}`
            ];
  
          if (!key1) {
            TRAFFIT_JOB_LOCATION[item.job_location.country] =
      ...
      const jobCategoryField = document.querySelector('select[name="query"]');
  
      for (let key in TRAFFIT_JOB_LOCATION) {
        geoField.add(new Option(key, TRAFFIT_JOB_LOCATION[key]));
      }
    ...
    var getInputs = function () {
      var inputs = Array.prototype.slice.call(
        filtersDiv.getElementsByTagName("input")
      );
      var selects = Array.prototype.slice.call(
        filtersDiv.getElementsByTagName("select")
      );
      var elements = [];
      ...
      callback: function (data) {
        ...
        for (var i = 0; i < items.length; i++) {
          ...
          var fillData = {
            ...
            awarded: items[i].awarded
              ? "traffit-an-list-xtxrxaxfxixtx2x0x1x6x-awarded"
              : "",
          };
          Object.assign(fillData, items[i]);
          fillData.date = formatDate(items[i].validStart);
          fillData.awarded = items[i].awarded
            ? "traffit-an-list-xtxrxaxfxixtx2x0x1x6x-awarded"
            : "";
          var geo =
          ...
          fillData.geolocation = geo.hasOwnProperty("locality")
            ...
            : "";
          fillData.geolocationHTML = fillData.geolocation
            ? `
              <div>
                  <i class='material-icons'>room</i>
                  <span>
                      ${fillData.geolocation}
                  </span>
              </div>
              `
            : "";
          ...
          <div>
              <i class='material-icons'>${
                items[i] && items[i].remote === "1" ? "my_location" : "push_pin"
              }
              </i>
              <span>
              ${t(
                items[i] && items[i].remote === "1"
                  ? "remoteTitle"
                  : "onSiteTitle"
              )}
              </span>
    ...
    var coreFunc = function () {
      ...
      if (!isWithinAts && scriptConfig.additionalStyles) {
        xxtraffitxxAppend(
          head,
          "<style>" + scriptConfig.additionalStyles + "</style>"
        );
      }
